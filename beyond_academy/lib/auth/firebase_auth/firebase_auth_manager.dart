...
class FirebaseAuthManager extends AuthManager
  ...
  Future resetPassword({
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('מייל לאיפוס סיסמה נשלח!')),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : BeyondAcademyFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' => 'אימייל זה כבר בשימוש על ידי משתמש אחר!',
        'INVALID_LOGIN_CREDENTIALS' => 'הפרטים שמולאו לא נכונים!',
        _ => 'שגיאה: [Error]',
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
